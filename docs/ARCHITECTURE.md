...
This document is a template until the target RTX 4060 workstation run pins a release and source commit. The final version must link every node to a file and line range in `reports/milestone1/source_map.json` and distinguish direct calls, ownership, inference, and not-observed behavior.
